...
import { getTripsInRange, printIncomeInDateRange, printNetProfitAndLossInDateRange, printTripsInDateRange } from "@/server/trips";
...
export function IncomeReportCard() {
    ...
    if (response?.success) {
      if (response?.data?.length ?? 0 > 0) {
        ...
        if (user?.success) {
          ...
          setIsLoading(true);
          const generateReportResponse = await printIncomeInDateRange(body);
          setIsLoading(false);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-xl">Income Report</CardTitle>
        <CardDescription>Export the income report</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
